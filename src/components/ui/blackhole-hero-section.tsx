...
vec3 starField(vec3 d) {
  ...
  col += vec3(0.013, 0.017, 0.030) * fbm(d * 2.6, 1.0) * clamp(uStars, 0.0, 1.0);
  return col;
}
...
void main() {
  ...
  // Calculate alpha based on luminance of the black hole & accretion glow
  float maxVal = max(c.r, max(c.g, c.b));
  float alpha = clamp(maxVal * 2.2, 0.0, 1.0);
...
export function BlackHoleHeroSection({
  ...
  return (
    <div
      ref={hostRef}
      className={`relative isolate h-full w-full overflow-visible bg-transparent ${className}`}
      {...rest}
    >
