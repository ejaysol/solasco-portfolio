...
export default function HorizonHeroBackground({ theme = 'dark' }: { theme?: 'dark' | 'light' }) {
  ...
  useEffect(() => {
    ...
    const initThree = () => {
      ...
      refs.scene.fog = new THREE.FogExp2(theme === 'light' ? 0xf8fafc : 0x000000, 0.00025);
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          time: { value: 0 },
          color1: { value: new THREE.Color(theme === 'light' ? 0xcbd5e1 : 0x0033ff) },
          color2: { value: new THREE.Color(theme === 'light' ? 0xf1f5f9 : 0xff0066) },
          opacity: { value: theme === 'light' ? 0.08 : 0.3 },
        },
        vertexShader: `
          varying vec2 vUv;
    ...
    const createMountains = () => {
      ...
      const layers = [
        { distance: -50, height: 60, color: isLight ? 0xcbd5e1 : 0x1a1a2e, opacity: 1 },
        { distance: -100, height: 80, color: isLight ? 0xe2e8f0 : 0x16213e, opacity: 0.8 },
        { distance: -150, height: 100, color: isLight ? 0xf1f5f9 : 0x0f3460, opacity: 0.6 },
        { distance: -200, height: 120, color: isLight ? 0xf8fafc : 0x0a4668, opacity: 0.4 },
      ];
  ...
  useEffect(() => {
    ...
    // 1. Fog Color
    if (refs.scene && refs.scene.fog) {
      const fogColor = isLight ? 0xf8fafc : 0x000000;
      (refs.scene.fog as THREE.FogExp2).color.setHex(fogColor);
    }

    // 2. Mountains Colors
    if (refs.mountains && refs.mountains.length > 0) {
      const colors = isLight
        ? [0xcbd5e1, 0xe2e8f0, 0xf1f5f9, 0xf8fafc]
        : [0x1a1a2e, 0x16213e, 0x0f3460, 0x0a4668];
      
      refs.mountains.forEach((mountain, i) => {
        const mat = mountain.material as THREE.MeshBasicMaterial;
        if (mat) {
          mat.color.setHex(colors[i] || 0x1a1a2e);
        }
      });
    }

    // 3. Nebula Shader Uniforms & Blending
    if (refs.nebula) {
      const mat = refs.nebula.material as THREE.ShaderMaterial;
      if (mat && mat.uniforms) {
        mat.uniforms.opacity.value = isLight ? 0.08 : 0.3;
        mat.uniforms.color1.value.setHex(isLight ? 0xcbd5e1 : 0x0033ff);
        mat.uniforms.color2.value.setHex(isLight ? 0xf1f5f9 : 0xff0066);
        mat.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
        mat.needsUpdate = true;
      }
    }

    // 4. Stars Opacity & Blending
    if (refs.stars && refs.stars.length > 0) {
      refs.stars.forEach((starField) => {
        const mat = starField.material as THREE.ShaderMaterial;
        if (mat) {
          mat.opacity = isLight ? 0.15 : 1.0;
          mat.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
          mat.needsUpdate = true;
        }
      });
    ...
    // 5. Post-processing Bloom Intensity adjustment
    if (refs.composer) {
      const bloomPass = refs.composer.passes.find(
        (pass) => pass instanceof UnrealBloomPass
      ) as UnrealBloomPass;
      if (bloomPass) {
        bloomPass.strength = isLight ? 0.15 : 0.8;
      }
    }
  }, [theme]);
  ...
  return (
    <div 
      className="fixed top-0 left-0 w-full h-full -z-10 pointer-events-none overflow-hidden transition-colors duration-300"
